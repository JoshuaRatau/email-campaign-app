                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('campaigns.update', $campaign) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="body" rows="5" class="form-control @error('body') is-invalid @enderror" required>{{ old('body', $campaign->body) }}</textarea>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="contact_list_id" class="form-select @error('contact_list_id') is-invalid @enderror" required>
                                @foreach($contactLists as $list)
                                    <option value="{{ $list->id }}" {{ old('contact_list_id', $campaign->contact_list_id) == $list->id ? 'selected' : '' }}>{{ $list->name }}</option>
                                @endforeach
                            </select>
